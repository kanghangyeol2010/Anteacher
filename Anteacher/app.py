...
from core.data_fetcher import fetch_stock_data, fetch_fred_data, fetch_news_rss, fetch_financial_metrics, fetch_global_news
...
from core.ai_orchestrator import generate_report, generate_quick_summary, generate_recommendation_reason, get_comprehensive_analysis
...
from core.financials import analyze_financials

# Load stock list
stock_universe = load_stock_universe()

# Cache AI report (deprecated - integrated into comprehensive analysis)
...
if not analyze_btn:
    ...
    with st.spinner("🔍 시장 대장주 중 상승 에너지가 강한 종목을 찾는 중..."):
        recommends = get_anteacher_recommendations(limit=3, cache_bust="v2")
    ...
    if recommends:
        ...
        for i, rec in enumerate(recommends):
            ...
            with cols[i]:
                ...
                # AI Reason (이미 생성된 데이터 사용)
                reason = rec.get('reason', '분석 중...')
                st.info(reason)
    else:
...
with st.spinner("🤖 AI가 열심히 분석 중입니다... 잠시만 기다려주세요!"):
    ...
    global_news = fetch_global_news(stock_name) if use_news else []
    ...
    # Consolidate AI analysis into one call
    try:
        macro_val_str = str(fred_df['value'].dropna().iloc[-1]) if fred_df is not None and not fred_df.empty and not fred_df['value'].dropna().empty else "N/A"
    except:
        macro_val_str = "N/A"
    
    # We need a cached wrapper for the complex analysis
    @st.cache_data(ttl=600)
    def cached_comprehensive_analysis(t, name, df, macro, l_news, g_news):
        return get_comprehensive_analysis(t, name, df, macro, l_news, g_news)

    if use_ai:
        with st.spinner("🧠 안테처가 시장 데이터와 뉴스를 종합 분석 중..."):
            ai_data = cached_comprehensive_analysis(ticker, stock_name, stock_df, macro_val_str, news_rss, global_news)
            sentiment_score = ai_data.get('sentiment', 0.0)
    else:
        ai_data = None
        sentiment_score = 0.0
...
if use_ai:
        with st.container():
            ...
            quick_summary = ai_data.get('quick_summary', '요약을 생성하지 못했습니다.')
...
# TABS
# ─────────────────────────────────────────
tab1, tab2, tab3, tab4 = st.tabs(["📐 수학적 분석", "📰 뉴스 리서치", "📝 안테처의 오답 노트", "💰 재무제표 분석"])
...
with tab2:
    ...
    if not news_rss:
        ...
        if global_news:
            st.markdown("### 🌎 Global Trends (해외 주요 뉴스)")
            for item in global_news[:5]:
                title = item.get('title', 'No Title')
                link  = item.get('url', '#')
                source = item.get('source', {}).get('name', 'Unknown')
                st.markdown(f"""
                <div class='news-card' style='border-color: #4a7ab5;'>
                    <a href='{link}' target='_blank' style='color: #82b1ff;'>{title}</a>
                    <div class='news-tag'>🌐 {source}</div>
                </div>
                """, unsafe_allow_html=True)
...
with tab3:
    ...
    if not use_ai or ai_data is None:
        st.info("AI 리포트 옵션이 꺼져 있거나 데이터를 불러올 수 없습니다.")
    else:
        report_main = ai_data.get('report_main', '리포트 본문을 생성하지 못했습니다.')
        wrong_note  = ai_data.get('wrong_note', '')
        summary     = ai_data.get('quick_summary', '')

        # ── 리포트 본문 표시
        st.markdown(f"<div class='teacher-box'>{report_main}</div>", unsafe_allow_html=True)
        ...
        if wrong_note:
            ...
            <div class='wrong-note'>
                ...
                <p>• 미분 기반 분석은 <b>후행 지표</b>입니다. 갑작스러운 외부 이슈에는 대응이 늦을 수 있어요.</p>
                <p>• 과거 데이터 기반 분석이므로 흐름을 이해하는 데 활용하세요.</p>
            </div>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════
with tab4:
    st.subheader(f"💰 {stock_name} 재무제표 분석")
    
    if not financial_metrics:
        st.warning("⚠️ 해당 종목의 재무 데이터를 불러올 수 없습니다.")
    else:
        # Calculate analysis
        analysis = analyze_financials(financial_metrics)
        
        # Display Metrics in Cards
        m1, m2, m3 = st.columns(3)
        with m1:
            per_val = f"{financial_metrics['per']:.2f}" if financial_metrics['per'] else "N/A"
            st.markdown(f"""
            <div style='background:#0d1f3c; border:1px solid #1e3a5f; border-radius:12px; padding:20px; text-align:center;'>
                <div style='color:#7da8d8; font-size:0.9rem;'>PER (주가수익비율)</div>
                <div style='color:#40c4ff; font-size:2rem; font-weight:700; margin:10px 0;'>{per_val}</div>
                <div style='color:#4a7ab5; font-size:0.75rem;'>수익 대비 주가 수준</div>
            </div>
            """, unsafe_allow_html=True)
        with m2:
            pbr_val = f"{financial_metrics['pbr']:.2f}" if financial_metrics['pbr'] else "N/A"
            st.markdown(f"""
            <div style='background:#0d1f3c; border:1px solid #1e3a5f; border-radius:12px; padding:20px; text-align:center;'>
                <div style='color:#7da8d8; font-size:0.9rem;'>PBR (주가순자산비율)</div>
                <div style='color:#40c4ff; font-size:2rem; font-weight:700; margin:10px 0;'>{pbr_val}</div>
                <div style='color:#4a7ab5; font-size:0.75rem;'>자산 대비 주가 수준</div>
            </div>
            """, unsafe_allow_html=True)
        with m3:
            roe_val = f"{financial_metrics['roe']*100:.2f}%" if financial_metrics['roe'] else "N/A"
            st.markdown(f"""
            <div style='background:#0d1f3c; border:1px solid #1e3a5f; border-radius:12px; padding:20px; text-align:center;'>
                <div style='color:#7da8d8; font-size:0.9rem;'>ROE (자기자본이익률)</div>
                <div style='color:#40c4ff; font-size:2rem; font-weight:700; margin:10px 0;'>{roe_val}</div>
                <div style='color:#4a7ab5; font-size:0.75rem;'>기업의 경영 효율성(IQ)</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;'>
            <div class='teacher-box'>
                <h4 style='color:#40c4ff; margin-top:0;'>① PER 분석</h4>
                <p>{analysis['per_text']}</p>
                <div style='font-size:0.8rem; color:#4a7ab5; background:rgba(0,0,0,0.2); padding:10px; border-radius:5px;'>
                    <b>공식:</b> 현재 주가 / 1주당 순이익 (EPS)
                </div>
            </div>
            <div class='teacher-box'>
                <h4 style='color:#40c4ff; margin-top:0;'>② PBR 분석</h4>
                <p>{analysis['pbr_text']}</p>
                <div style='font-size:0.8rem; color:#4a7ab5; background:rgba(0,0,0,0.2); padding:10px; border-radius:5px;'>
                    <b>공식:</b> 현재 주가 / 1주당 순자산 (BPS)
                </div>
            </div>
        </div>
        
        <div style='margin-top: 20px;'>
            <div class='teacher-box' style='border-left: 5px solid #ab47bc;'>
                <h4 style='color:#ce93d8; margin-top:0;'>③ ROE 분석</h4>
                <p>{analysis['roe_text']}</p>
                <div style='font-size:0.8rem; color:#4a7ab5; background:rgba(0,0,0,0.2); padding:10px; border-radius:5px;'>
                    <b>공식:</b> (당기순이익 / 자기자본) × 100
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if analysis['summary']:
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#1b3d2f,#0d2b1f);
                        border:2px solid #66bb6a; border-radius:14px;
                        padding:24px; margin-top:25px;'>
                {analysis['summary']}
            </div>
            """, unsafe_allow_html=True)
            
    st.caption("⚠️ 재무 제표 데이터는 최근 결산 기준이며, 실시간 주가에 따라 PER/PBR은 변동될 수 있습니다.")
